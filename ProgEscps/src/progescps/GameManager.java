 ...
 *
 * Extended with save/load slot support:
 * - Saves are written to "saves/<filename>" if using the UI save dialog.
 * - Legacy "savegame.dat" in the app root is still supported.
 *
 * New public methods:
 * - listSaveFiles() : List<String> — synchronous, returns available save filenames.
 * - uiSaveGameTo(String) : queue save to named slot (background thread)
 * - uiLoadGameFrom(String) : load named slot (background thread)
 */
